<div class="col-sm-10 col-sm-offset-1 text-center panel panel-default top-padding-md bottom-padding-md bottom-margin-none">
    ...
    @push('scripts_ready')
        ...
        incident_map.setView(new L.LatLng(38.8282,-96.5795),4);
        incident_map.addLayer(osm);

        @cache($geo_data)
            @foreach($search ? $incidents : $geo_data as $geo_datum)
                var marker_{{$geo_datum->id}} = L.marker([{{ $geo_datum->latitude }},{{ $geo_datum->longitude }}]).addTo(incident_map);                    
